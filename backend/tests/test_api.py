...
def test_update_order_status_api_success(client):
    ...
    assert response.status_code == 200
    assert response.json['order_id'] == "UPDATE001"
    assert response.json['item_name'] == "Test Item"
    assert response.json['quantity'] == 1
    assert response.json['customer_id'] == "C1"
...
def test_list_all_orders_api_with_data(client):
    ...
    assert response.json[0]['order_id'] == "LST001"
    assert response.json[0]['item_name'] == "Item A"
    assert response.json[0]['quantity'] == 1
    assert response.json[0]['customer_id'] == "C1"
    assert response.json[0]['status'] == "pending"
    assert response.json[1]['order_id'] == "LST002"
    assert response.json[1]['item_name'] == "Item B"
    assert response.json[1]['quantity'] == 2
    assert response.json[1]['customer_id'] == "C2"
...
def test_list_orders_by_status_api_matching(client):
    ...
    assert response.json[0]['item_name'] == "A"
    assert response.json[0]['quantity'] == 1
    assert response.json[0]['customer_id'] == "C1"
    assert response.json[0]['status'] == "pending"
